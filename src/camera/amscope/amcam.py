...
AMCAM_FLAG_DDR = (
    0x02000000  # use very large capacity DDR (Double Data Rate SDRAM) for frame buffer
)
...
AMCAM_FLAG_LEVELRANGE_HARDWARE = (
    0x0000020000000000  # hardware level range, put(get)_LevelRangeV2
)
AMCAM_FLAG_EVENT_HARDWARE = (
    0x0000040000000000  # hardware event, such as exposure start & stop
)
...
AMCAM_EVENT_STILLIMAGE = (
    0x0005  # snap (still) frame arrived, use Amcam_PullStillImage to get this frame
)
...
AMCAM_OPTION_NOFRAME_TIMEOUT = (
    0x01  # no frame timeout: 1 = enable; 0 = disable. default: disable
)
AMCAM_OPTION_THREAD_PRIORITY = (
    0x02  # set the priority of the internal thread which grab data from the usb device.
)
...
AMCAM_OPTION_COLORMATIX = (
    0x0D  # enable or disable the builtin color matrix, default value: 1
)
AMCAM_OPTION_WBGAIN = (
    0x0E  # enable or disable the builtin white balance gain, default value: 1
)
...
#      3: Gain Preferred
#      default value: 1
AMCAM_OPTION_FRAMERATE = (
    0x11  # limit the frame rate, range=[0, 63], the default value 0 means no limit
)
...
#         ((val & 0xff00) >> 8): sequence
#         ((val & 0xff0000) >> 8): average number
AMCAM_OPTION_SHARPENING = (
    0x1E  # Sharpening: (threshold << 24) | (radius << 16) | strength)
)
#     strength: [0, 500], default: 0 (disable)
#     radius: [1, 10]
#     threshold: [0, 255]
AMCAM_OPTION_FACTORY = 0x1F  # restore the factory settings
AMCAM_OPTION_TEC_VOLTAGE = (
    0x20  # get the current TEC voltage in 0.1V, 59 mean 5.9V; readonly
)
...
AMCAM_OPTION_AUTOEXP_THRESHOLD = (
    0x29  # threshold of auto exposure, default value: 5, range = [2, 15]
)
...
AMCAM_OPTION_PRECISE_FRAMERATE = (
    0x2D  # precise frame rate current value in 0.1 fps, range:[1~maximum]
)
...
AMCAM_OPTION_FRONTEND_DEQUE_LENGTH = (
    0x31  # frontend frame buffer deque length, range: [2, 1024], default: 3
)
AMCAM_OPTION_FRAME_DEQUE_LENGTH = 0x31  # alias of AMCAM_OPTION_FRONTEND_DEQUE_LENGTH
AMCAM_OPTION_MIN_PRECISE_FRAMERATE = (
    0x32  # precise frame rate minimum value in 0.1 fps, such as 15 means 1.5 fps
)
...
AMCAM_OPTION_LOW_NOISE = (
    0x38  # low noise mode (Higher signal noise ratio, lower frame rate): 1 => enable
)
...
#      enable(1):  others
AMCAM_OPTION_FLUSH = (
    0x3D  # 1 = hard flush, discard frames cached by camera DDR (if any)
)
...
AMCAM_OPTION_DEFECT_PIXEL = (
    0x40  # Defect Pixel Correction: 0 => disable, 1 => enable; default: 1
)
...
AMCAM_IOCONTROLTYPE_GET_SUPPORTEDMODE = (
...
AMCAM_IOCONTROLTYPE_GET_DEBOUNCERTIME = (
    0x0B  # debouncer time in microseconds, [0, 20000]
)
...
AMCAM_IOCONTROLTYPE_GET_TRIGGERDELAY = (
    0x0F  # Trigger delay time in microseconds, [0, 5000000]
)
...
AMCAM_IOCONTROLTYPE_GET_COUNTERSOURCE = (
    0x13  # 0x00-> Opto-isolated input, 0x01-> GPIO0, 0x02-> GPIO1
)
...
AMCAM_IOCONTROLTYPE_GET_PWMSOURCE = (
    0x1D  # 0x00-> Opto-isolated input, 0x01-> GPIO0, 0x02-> GPIO1
)
...
AMCAM_IOCONTROLTYPE_GET_STROBEDELAYMODE = (
    0x21  # boolean, 1 -> delay, 0 -> pre-delay; compared to exposure active signal
)
AMCAM_IOCONTROLTYPE_SET_STROBEDELAYMODE = 0x22
AMCAM_IOCONTROLTYPE_GET_STROBEDELAYTIME = (
    0x23  # Strobe delay or pre-delay time in microseconds, [0, 5000000]
)
AMCAM_IOCONTROLTYPE_SET_STROBEDELAYTIME = 0x24
AMCAM_IOCONTROLTYPE_GET_STROBEDURATION = (
    0x25  # Strobe duration time in microseconds, [0, 5000000]
)
...
AMCAM_IOCONTROLTYPE_GET_UART_BAUDRATE = (
    0x2B  # baud rate: 0-> 9600; 1-> 19200; 2-> 38400; 3-> 57600; 4-> 115200
)
AMCAM_IOCONTROLTYPE_SET_UART_BAUDRATE = 0x2C
AMCAM_IOCONTROLTYPE_GET_UART_LINEMODE = (
    0x2D  # line mode: 0-> TX(GPIO_0)/RX(GPIO_1); 1-> TX(GPIO_1)/RX(GPIO_0)
)
...
class AmcamAfParam:
    def __init__(self, imax, imin, idef, imaxabs, iminabs, zoneh, zonev):
        ...
        self.imaxabs = (
            imaxabs  # maximum absolute auto focus sensor board positon, micrometer
        )
        self.iminabs = (
            iminabs  # maximum absolute auto focus sensor board positon, micrometer
        )
...
if sys.platform == "win32":
    ...
    class _ModelV2(ctypes.Structure):  # camera model v2 win32
        _fields_ = [
            ...
            (
                "preview",
                ctypes.c_uint,
            ),  # number of preview resolution, same as Amcam_get_ResolutionNumber()
            (
                "still",
                ctypes.c_uint,
            ),  # number of still resolution, same as Amcam_get_StillResolutionNumber()
    ...
    class _DeviceV2(ctypes.Structure):  # win32
        _fields_ = [
            ...
            (
                "id",
                ctypes.c_wchar * 64,
            ),  # unique and opaque id of a connected camera, for Amcam_Open
    ...
    class _ModelV2(ctypes.Structure):  # camera model v2 linux/mac
        _fields_ = [
            ("name", ctypes.c_char_p),  # model name, in Windows, we use unicode
            ("flag", ctypes.c_ulonglong),  # AMCAM_FLAG_xxx, 64 bits
            (
                "maxspeed",
                ctypes.c_uint,
...
class Amcam:
    ...
    class __AfParam(ctypes.Structure):
        _fields_ = [
            ("imax", ctypes.c_int),  # maximum auto focus sensor board positon
            ("imin", ctypes.c_int),  # minimum auto focus sensor board positon
            ("idef", ctypes.c_int),  # conjugate calibration positon
            (
                "imaxabs",
                ctypes.c_int,
            ),  # maximum absolute auto focus sensor board positon, micrometer
            (
                "iminabs",
                ctypes.c_int,
            ),  # maximum absolute auto focus sensor board positon, micrometer
    ...
    def put_GlobalOption(cls, iOption, iValue):
        cls.__initlib()
        return cls.__lib.Amcam_Version(
            None, ctypes.c_uint(iOption), ctypes.c_int(iValue)
        )
    ...
    def __convertModel(a):
        t = AmcamModelV2(
            ...
            a.xpixsz,
            a.ypixsz,
            __class__.__convertResolution(a),
        )
        return t

    @staticmethod
    def __convertDevice(a):
        return AmcamDeviceV2(
            __class__.__convertStr(a.displayname),
            __class__.__convertStr(a.id),
    ...
    def HotPlug(cls, fun):
        if (
            ...
            if cls.__hotplug is None:
                ...
                cls.__lib.Amcam_HotPlug(
                    cls.__HOTPLUG_CALLBACK(cls.__hotplugCallbackFun), None
                )
    ...
    def StartPullModeWithCallback(self, fun, ctx):
        ...
        self.__lib.Amcam_StartPullModeWithCallback(
            self.__h, self.__cb, ctypes.py_object(self)
        )
    ...
    def PullImageWithRowPitchV2(self, pImageData, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Amcam_PullImageWithRowPitchV2(
                self.__h, pImageData, bits, rowPitch, None
            )
        else:
            x = __FrameInfoV2()
            self.__lib.Amcam_PullImageWithRowPitchV2(
                self.__h, pImageData, bits, rowPitch, byref(x)
            )
            self.__convertFrameInfo(pInfo, x)

    def PullStillImageWithRowPitchV2(self, pImageData, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Amcam_PullStillImageWithRowPitchV2(
                self.__h, pImageData, bits, rowPitch, None
            )
        else:
            x = __FrameInfoV2()
            self.__lib.Amcam_PullStillImageWithRowPitchV2(
                self.__h, pImageData, bits, rowPitch, pInfo
            )
            self.__convertFrameInfo(pInfo, x)
    ...
    def SnapN(self, nResolutionIndex, nNumber):
        self.__lib.Amcam_SnapN(
            self.__h, ctypes.c_uint(nResolutionIndex), ctypes.c_uint(nNumber)
        )
    ...
    def get_Resolution(self, nResolutionIndex):
        x = ctypes.c_int(0)
        y = ctypes.c_int(0)
        self.__lib.Amcam_get_Resolution(
            self.__h, ctypes.c_uint(nResolutionIndex), ctypes.byref(x), ctypes.byref(y)
        )
        return (x.value, y.value)
    ...
    def get_PixelSize(self, nResolutionIndex):
        x = ctypes.c_float(0)
        y = ctypes.c_float(0)
        self.__lib.Amcam_get_PixelSize(
            self.__h, ctypes.c_uint(nResolutionIndex), ctypes.byref(x), ctypes.byref(y)
        )
        return (x.value, y.value)
    ...
    def get_ResolutionRatio(self, nResolutionIndex):
        x = ctypes.c_int(0)
        y = ctypes.c_int(0)
        self.__lib.Amcam_get_ResolutionRatio(
            self.__h, ctypes.c_uint(nResolutionIndex), ctypes.byref(x), ctypes.byref(y)
        )
        return (x.value, y.value)
    ...
    def put_MaxAutoExpoTimeAGain(self, maxTime, maxAGain):
        return self.__lib.Amcam_put_MaxAutoExpoTimeAGain(
            self.__h, ctypes.c_uint(maxTime), ctypes.c_ushort(maxAGain)
        )

    def get_MaxAutoExpoTimeAGain(self):
        x = ctypes.c_uint(0)
        y = ctypes.c_ushort(0)
        self.__lib.Amcam_get_MaxAutoExpoTimeAGain(
            self.__h, ctypes.byref(x), ctypes.byref(y)
        )
        return (x.value, y.value)

    def put_MinAutoExpoTimeAGain(self, minTime, minAGain):
        self.__lib.Amcam_put_MinAutoExpoTimeAGain(
            self.__h, ctypes.c_uint(minTime), ctypes.c_ushort(minAGain)
        )

    def get_MinAutoExpoTimeAGain(self):
        x = ctypes.c_uint(0)
        y = ctypes.c_ushort(0)
        self.__lib.Amcam_get_MinAutoExpoTimeAGain(
            self.__h, ctypes.byref(x), ctypes.byref(y)
        )
        return (x.value, y.value)
    ...
    def get_ExpTimeRange(self):
        ...
        y = ctypes.c_uint(0)
        z = ctypes.c_uint(0)
        self.__lib.Amcam_get_ExpTimeRange(
            self.__h, ctypes.byref(x), ctypes.byref(y), ctypes.byref(z)
        )
    ...
    def get_ExpoAGainRange(self):
        ...
        y = ctypes.c_ushort(0)
        z = ctypes.c_ushort(0)
        self.__lib.Amcam_get_ExpoAGainRange(
            self.__h, ctypes.byref(x), ctypes.byref(y), ctypes.byref(z)
        )
        return (x.value, y.value, z.value)
    ...
    def put_TempTint(self, nTemp, nTint):
        self.__lib.Amcam_put_TempTint(
            self.__h, ctypes.c_int(nTemp), ctypes.c_int(nTint)
        )
    ...
    def get_StillResolution(self, nResolutionIndex):
        x = ctypes.c_int(0)
        y = ctypes.c_int(0)
        self.__lib.Amcam_get_StillResolution(
            self.__h, ctypes.c_uint(nResolutionIndex), ctypes.byref(x), ctypes.byref(y)
        )
        return (x.value, y.value)
    ...
    def put_LEDState(self, iLed, iState, iPeriod):
        self.__lib.Amcam_put_LEDState(
            self.__h,
            ctypes.c_ushort(iLed),
            ctypes.c_ushort(iState),
            ctypes.c_ushort(iPeriod),
        )

    def write_EEPROM(self, addr, pBuffer):
        self.__lib.Amcam_write_EEPROM(
            self.__h, addr, pBuffer, ctypes.c_uint(len(pBuffer))
        )

    def read_EEPROM(self, addr, pBuffer):
        self.__lib.Amcam_read_EEPROM(
            self.__h, addr, pBuffer, ctypes.c_uint(len(pBuffer))
        )

    def write_Pipe(self, pipeNum, pBuffer):
        self.__lib.Amcam_write_Pipe(
            self.__h, pipeNum, pBuffer, ctypes.c_uint(len(pBuffer))
        )

    def read_Pipe(self, pipeNum, pBuffer):
        self.__lib.Amcam_read_Pipe(
            self.__h, pipeNum, pBuffer, ctypes.c_uint(len(pBuffer))
        )
    ...
    def put_Option(self, iOption, iValue):
        self.__lib.Amcam_put_Option(
            self.__h, ctypes.c_uint(iOption), ctypes.c_int(iValue)
        )
    ...
    def put_Roi(self, xOffset, yOffset, xWidth, yHeight):
        self.__lib.Amcam_put_Roi(
            self.__h,
            ctypes.c_uint(xOffset),
            ctypes.c_uint(yOffset),
            ctypes.c_uint(xWidth),
    ...
    def get_Roi(self):
        ...
        w = ctypes.c_uint(0)
        h = ctypes.c_uint(0)
        self.__lib.Amcam_get_Roi(
            self.__h, ctypes.byref(x), ctypes.byref(y), ctypes.byref(w), ctypes.byref(h)
        )
    ...
    def get_FrameRate(self):
        ...
        y = ctypes.c_uint(0)
        z = ctypes.c_uint(0)
        self.__lib.Amcam_get_FrameRate(
            self.__h, ctypes.byref(x), ctypes.byref(y), ctypes.byref(z)
        )
        return (x.value, y.value, z.value)
    ...
    def Update(cls, camId, filePath, pFun):
        cls.__progress = pFun
        if sys.platform == "win32":
            return cls.__lib.Amcam_Update(
                camId,
                filePath,
                cls.__PROGRESS_CALLBACK(cls.__progressCallbackFun),
                None,
            )
        else:
            return cls.__lib.Amcam_Update(
    ...
    def __initlib(cls):
        if cls.__lib is None:
            try:
                ...
                if sys.platform == "win32":
                    cls.__lib = ctypes.windll.LoadLibrary(
                        os.path.join(dir, "amcam.dll")
                    )
                elif sys.platform.startswith("linux"):
                    cls.__lib = ctypes.cdll.LoadLibrary(
                        os.path.join(dir, "libamcam.so")
                    )
                else:
                    cls.__lib = ctypes.cdll.LoadLibrary(
                        os.path.join(dir, "libamcam.dylib")
                    )
            except OSError:
                pass
            ...
            if sys.platform == "win32":
                ...
                cls.__lib.Amcam_Update.argtypes = [
                    ctypes.c_wchar_p,
                    ctypes.c_wchar_p,
                    cls.__PROGRESS_CALLBACK,
                    ctypes.py_object,
                ]
            else:
            ...
            cls.__lib.Amcam_StartPullModeWithCallback.argtypes = [
                ctypes.c_void_p,
                cls.__EVENT_CALLBACK,
                ctypes.py_object,
            ]
            cls.__lib.Amcam_PullImageV2.restype = ctypes.c_int
            cls.__lib.Amcam_PullImageV2.errcheck = cls.__errcheck
            cls.__lib.Amcam_PullImageV2.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char_p,
                ctypes.c_int,
            ...
            cls.__lib.Amcam_PullStillImageV2.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char_p,
                ctypes.c_int,
                ctypes.POINTER(cls.__FrameInfoV2),
            ...
            cls.__lib.Amcam_PullImageWithRowPitchV2.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char_p,
                ctypes.c_int,
                ctypes.c_int,
            ...
            cls.__lib.Amcam_PullStillImageWithRowPitchV2.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char_p,
                ctypes.c_int,
                ctypes.c_int,
            ...
            cls.__lib.Amcam_SnapN.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_uint,
            ]
            ...
            cls.__lib.Amcam_put_Size.argtypes = [
                ctypes.c_void_p,
                ctypes.c_int,
                ctypes.c_int,
            ]
            ...
            cls.__lib.Amcam_get_Size.argtypes = [
                ctypes.c_void_p,
            ...
            cls.__lib.Amcam_get_Resolution.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.POINTER(ctypes.c_int),
                ctypes.POINTER(ctypes.c_int),
            ...
            cls.__lib.Amcam_get_ResolutionRatio.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.POINTER(ctypes.c_int),
                ctypes.POINTER(ctypes.c_int),
            ...
            cls.__lib.Amcam_get_RawFormat.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_uint),
                ctypes.POINTER(ctypes.c_uint),
            ]
            ...
            cls.__lib.Amcam_put_MaxAutoExpoTimeAGain.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_ushort,
            ]
            ...
            cls.__lib.Amcam_get_MaxAutoExpoTimeAGain.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_uint),
                ctypes.POINTER(ctypes.c_ushort),
            ]
            ...
            cls.__lib.Amcam_put_MinAutoExpoTimeAGain.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_ushort,
            ]
            ...
            cls.__lib.Amcam_get_MinAutoExpoTimeAGain.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_uint),
                ctypes.POINTER(ctypes.c_ushort),
            ]
            ...
            cls.__lib.Amcam_AwbOnce.argtypes = [
                ctypes.c_void_p,
                ctypes.c_void_p,
                ctypes.c_void_p,
            ]
            ...
            cls.__lib.Amcam_AwbInit.argtypes = [
                ctypes.c_void_p,
                ctypes.c_void_p,
                ctypes.c_void_p,
            ]
            ...
            cls.__lib.Amcam_put_TempTint.argtypes = [
                ctypes.c_void_p,
                ctypes.c_int,
                ctypes.c_int,
            ]
            ...
            cls.__lib.Amcam_get_TempTint.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Hue.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Saturation.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Brightness.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Contrast.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Gamma.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Chrome.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_VFlip.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_HFlip.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Negative.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_Speed.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_ushort),
            ]
            ...
            cls.__lib.Amcam_get_HZ.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_get_StillResolution.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.POINTER(ctypes.c_int),
                ctypes.POINTER(ctypes.c_int),
            ...
            cls.__lib.Amcam_get_RealTime.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_put_LevelRange.argtypes = [
                ctypes.c_void_p,
                (ctypes.c_ushort * 4),
                (ctypes.c_ushort * 4),
            ]
            ...
            cls.__lib.Amcam_get_LevelRange.argtypes = [
                ctypes.c_void_p,
                (ctypes.c_ushort * 4),
                (ctypes.c_ushort * 4),
            ]
            ...
            cls.__lib.Amcam_put_LevelRangeV2.argtypes = [
                ctypes.c_void_p,
                ctypes.c_ushort,
                ctypes.POINTER(cls.__RECT),
                (ctypes.c_ushort * 4),
            ...
            cls.__lib.Amcam_get_LevelRangeV2.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_ushort),
                ctypes.POINTER(cls.__RECT),
                (ctypes.c_ushort * 4),
            ...
            cls.__lib.Amcam_put_LEDState.argtypes = [
                ctypes.c_void_p,
                ctypes.c_ushort,
                ctypes.c_ushort,
                ctypes.c_ushort,
            ...
            cls.__lib.Amcam_write_EEPROM.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_char_p,
                ctypes.c_uint,
            ...
            cls.__lib.Amcam_read_EEPROM.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_char_p,
                ctypes.c_uint,
            ...
            cls.__lib.Amcam_read_Pipe.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_char_p,
                ctypes.c_uint,
            ...
            cls.__lib.Amcam_write_Pipe.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_char_p,
                ctypes.c_uint,
            ...
            cls.__lib.Amcam_put_Option.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_int,
            ]
            ...
            cls.__lib.Amcam_get_Option.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.POINTER(ctypes.c_int),
            ]
            ...
            cls.__lib.Amcam_put_Roi.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_uint,
                ctypes.c_uint,
            ...
            cls.__lib.Amcam_get_Roi.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_uint),
                ctypes.POINTER(ctypes.c_uint),
                ctypes.POINTER(ctypes.c_uint),
            ...
            cls.__lib.Amcam_get_AfParam.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(cls.__AfParam),
            ]
            cls.__lib.Amcam_IoControl.restype = ctypes.c_int
            cls.__lib.Amcam_IoControl.errcheck = cls.__errcheck
            cls.__lib.Amcam_IoControl.argtypes = [
                ctypes.c_void_p,
                ctypes.c_uint,
                ctypes.c_uint,
                ctypes.c_int,
            ...
            cls.__lib.Amcam_read_UART.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char_p,
                ctypes.c_uint,
            ]
            ...
            cls.__lib.Amcam_write_UART.argtypes = [
                ctypes.c_void_p,
                ctypes.c_char_p,
                ctypes.c_uint,
            ]
            ...
            cls.__lib.Amcam_put_Linear.argtypes = [
                ctypes.c_void_p,
            ...
            cls.__lib.Amcam_put_Curve.argtypes = [
                ctypes.c_void_p,
                ctypes.POINTER(ctypes.c_ubyte),
                ctypes.POINTER(ctypes.c_ushort),
            ]
            ...
            if sys.platform != "win32":
                ...
                cls.__lib.Amcam_HotPlug.argtypes = [
                    cls.__HOTPLUG_CALLBACK,
                    ctypes.c_void_p,
                ]
