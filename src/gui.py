...
from lib.dlpc import *

# importing utilities
from lib.globals import (
    DEFAULT_RED_EXPOSURE,
    DEFAULT_UV_EXPOSURE,
    THUMBNAIL_SIZE,
    EventDispatcher,
    compute_focus_score,
...
from lib.gui import FloatEntry, IntEntry, Thumbnail
from lib.img import image_to_tk_image
from lib.structs import *
from lib.tiling_utils import *
...
#     img_lapl = (
#         np.abs(cv2.Sobel(img, cv2.CV_64F, 1, 0, ksize=1)) + np.abs(cv2.Sobel(img, cv2.CV_64F, 0, 1, ksize=1))
#     ) / mean
#     if save:
...
class EventDispatcherOld:
    ...
    def _sync_led_enable(self, mask: int) -> None:
        """
        Sync the Image channel mask to the DLPC chip's LED enable mask to
        hard turn on/off LEDs to avoid exposure due to 'uv leakage'.
        """
    ...
    def move_absolute(self, coords: dict[str, float]):
        ...
        # in work position, the x moves in negative direction (away from home)
        # and y moves in positive direction (away from home)
        x = coords.get("x", 0)
        y = coords.get("y", 0)
        z = coords.get("z", 0)
        set_point = (x, y, z)
    ...
    def _check_bounds(self, set_point):
        ...
        for name, i in axes:
            ...
            val = set_point[i]
            if not (lo <= val <= hi):
                return False, (f"Moving {name.upper()} to {val} prohibited. Boundaries are [{lo}, {hi}]")
        return True, None
    ...
    def autofocus(self, blue_only, log=False):
        ...
        if self.hardware.stage.has_homing():
            ...
            if blue_only:
                z_base += 50.0
                if not (self.move_absolute({"z": z_base})):
...
class StagePositionFrame:
    def __init__(self, parent, event_dispatcher: EventDispatcher, uvmode, ui_scale: float = 1.0):
        ...
        self.chip_origin_button = ttk.Button(
            self.shortcut_frame,
            text="Chip origin",
            command=on_chip_origin,
            state=btn_state,
        )
        self.chip_origin_button.grid(row=0, column=0, padx=5)

        self.chip_unload_button = ttk.Button(
            self.shortcut_frame,
            text="Load/unload",
            command=on_chip_unload,
            state=btn_state,
        )
    ...
    def create_custom_xyz_control(self, parent):
        ...
        self.custom_step_pos_x_button = ttk.Button(
            parent,
            text="+X",
            command=lambda: custom_step_move(Direction.POS, Axis.X, self.custom_step_size.get()),
        )
        self.custom_step_pos_y_button = ttk.Button(
            parent,
            text="+Y",
            command=lambda: custom_step_move(Direction.POS, Axis.Y, self.custom_step_size.get()),
        )
        self.custom_step_pos_z_button = ttk.Button(
            parent,
            text="+Z",
            command=lambda: custom_step_move(Direction.POS, Axis.Z, self.custom_step_size.get()),
        )

        self.custom_step_neg_x_button = ttk.Button(
            parent,
            text="-X",
            command=lambda: custom_step_move(Direction.NEG, Axis.X, self.custom_step_size.get()),
        )
        self.custom_step_neg_y_button = ttk.Button(
            parent,
            text="-Y",
            command=lambda: custom_step_move(Direction.NEG, Axis.Y, self.custom_step_size.get()),
        )
        self.custom_step_neg_z_button = ttk.Button(
            parent,
            text="-Z",
            command=lambda: custom_step_move(Direction.NEG, Axis.Z, self.custom_step_size.get()),
        )
    ...
    def create_xy_control(self, parent, ui_scale: float = 1.0):
        ...
        self.xy_canvas = tkinter.Canvas(parent, width=1, height=1, bg='#f0f0f0', highlightthickness=0)
        self.xy_canvas.pack()
    ...
    def _draw_xy(self, ui_scale: float):
        ...
        for i, (radius, step) in enumerate(zip(self._xy_radii, step_sizes)):
            ...
            self.xy_canvas.create_text(center, y_pos, text=str(step), fill='white', font=font_small)
    ...
    def create_z_control(self, parent, ui_scale: float = 1.0):
        ...
        self.z_canvas = tkinter.Canvas(parent, width=1, height=1, bg='#34495e', highlightthickness=0)
        self.z_canvas.pack()
    ...
    def _draw_z(self, ui_scale: float):
        ...
        for i in range(8):
            ...
            rect_id = self.z_canvas.create_rectangle(
                0, y_start, bar_width, y_end, fill=fill, outline='#2c3e50', width=2
            )
    ...
    def __init__(self, parent, event_dispatcher: EventDispatcher):
        ...
        for i, coord in ((0, "x"), (1, "y"), (2, "ϴ")):
            if i == 0 or i == 1:
                ...
                self.position_intputs.append(FloatEntry(parent=self.absolute_frame, default=0))
            self.position_intputs[-1].widget.grid(row=0, column=i)

        def callback_set():
            x, y, t = self._position()
            print(f"setting image position: {x}, {y}, {t}")
    ...
    def __init__(self, parent, button_text, import_command, predefined_images=None):
        ...
        if predefined_images:
            ...
            self.selector = PredefinedImageSelector(
                self.frame, THUMBNAIL_SIZE, predefined_images, on_select=import_command
            )
...
class PatternDisplayFrame:  # read only pattern display in red and uv focusing mode
    def __init__(self, parent, event_dispatcher: EventDispatcher):
        ...
        ttk.Label(
            self.pattern_display_frame,
            text="(Upload in Pattern Upload tab)",
...
class ExposureFrame:
    def __init__(self, parent, event_dispatcher: EventDispatcher):
        ...
        def on_change_exposure(new_exposure):
            self.exposure_time_entry.set(new_exposure)

        event_dispatcher.add_event_listener(Event.EXPOSURE_TIME_CHANGED, on_change_exposure)
...
class PatternUploadFrame:
    def __init__(self, parent, event_dispatcher: EventDispatcher):
        ...
        self.pattern_selector = ImageSelectFrame(self.upload_frame, "Select Pattern Image", self._on_pattern_upload)
        ...
        ttk.Label(self.info_frame, textvariable=self.pattern_path_var, foreground="blue").grid(
        ...
        # Upload instructions
        instruction_text = (
            "Upload your pattern image using the selector above. This pattern will be used in UV mode for lithography."
        )
        ttk.Label(self.upload_frame, text=instruction_text, wraplength=300).grid(row=1, column=0, padx=5, pady=5)
    ...
    def _on_pattern_upload(self, _):
        ...
        if self.pattern_selector.thumb.image:
            ...
            self.event_dispatcher.set_pattern_image(self.pattern_selector.thumb.image, self.pattern_selector.thumb.path)
...
class GlobalSettingsFrame:
    def __init__(self, parent, event_dispatcher: EventDispatcher, enable_detection: bool = False):
        ...
        self.detect_button = ttk.Button(
            self.frame,
            text="Detect Fiducials",
            command=lambda: event_dispatcher.detect_marks_for_slam(
                event_dispatcher.camera_image, event_dispatcher.model
            ),
        )
...
class ExposureHistoryFrame:
    ...
    def _refresh(self):
        ...
        for exp_log in self.event_dispatcher.exposure_history[-10:]:
            ...
            line = f"{t} {exp_log.path} {int(exp_log.duration)}ms \
            X:{exp_log.coords[0]} Y:{exp_log.coords[1]} Z:{exp_log.coords[2]}\n"
...
class ProjectorDisplayFrame:
    ...
    def __init__(
        ...
        parent,
        event_dispatcher: EventDispatcher,
        display_size: tuple[int, int] = (320, 180),
    ):
        self.frame = ttk.Frame(parent)
        self.event_dispatcher = event_dispatcher
...
class TilingFrame:
    def __init__(self, parent, model: EventDispatcher):
        ...
        self.params.ratio = 0.5  # 1/ratio = number of steps in between current
        # tile to next tile to take --> used to define stride size
        ...
        self.layer = tkinter.IntVar(self.frame, value=1)  # default is layer 1

        def on_layer_change(*args):
            try:
                # Attempt to get the integer value
                current_layer = self.layer.get()
                if current_layer == 1:
                    self.model.on_event(Event.EXPOSURE_TIME_CHANGED, self.exposure_time)
                else:
                    self.model.on_event(Event.EXPOSURE_TIME_CHANGED, 8000)
                self.layer.set(current_layer)
                print(f"on_layer_change: {current_layer}")

            except tkinter.TclError:
                pass

        self.layer.trace_add("write", on_layer_change)
        ...
        def snake_pattern_alignment_errors(
            model,
            dest_img,
            src_img,
            dir_x=None,
            ...
            """
            Takes in 2 images:
            - `src_img`: original image (either former location or pattern image)
            - `dest_img`: new image (either new offset or raw red focus)
            - `dir_x, dir_y`: x direction, y direction stage moved
            - 'step_x, step_y`: in stepper steps, how many taken per axis
            - `src_pattern, dest_pattern`: booleans that indicate whether image is digital (False --> camera)
            ...
            """
            assert src_img is not None and dest_img is not None, "Error: one or both images are None"
            assert (step_x >= 0) and (step_y >= 0), (
                "Error: step size cannot be negative, use direction args for negatives"
            )
            # calculate step size
            step_size_x_px = round(
                step_x / self.params.px_to_step_x
            )  # convert steps to pixels => modified to be other way around
            step_size_y_px = round(step_y / self.params.px_to_step_y)  # convert steps to pixels
            print(
                f"step size calculations (pixels): dir_x {dir_x}, dir_y {dir_y}, \
                step_size_x_px {step_size_x_px}, step_size_y_px {step_size_y_px}"
            )

            # pre-processing
            dest_processed, d_h, d_w = rf_detr_preprocess(dest_img, layer + 1 if dest_pattern is True else layer)
            src_processed, s_h, s_w = rf_detr_preprocess(src_img, layer + 1 if src_pattern is True else layer)
            ...
            src_marks_raw = detect_marks_for_slam(
                src_processed, model, d_h, d_w, CONFIDENCE_THRESHOLD
            )  # assume returning [dict{}]
            dest_marks_raw = detect_marks_for_slam(
                dest_processed, model, s_h, s_w, CONFIDENCE_THRESHOLD
            )  # assume returning [dict{}]
            print(
                "detected src mark count: ",
                len(src_marks_raw),
                "detected dest mark count: ",
                len(dest_marks_raw),
            )
            ...
            print(f"raw dest_marks: {dest_marks}")
            print(f"raw src_marks: {src_marks}")

            # if direction and step size are defined, then we should offset the src
            # img and bring it close to dest markers and match them, this maximizes
            # the probability that we find correct matches without needing to run some
            # crazy algorithm that takes forever ;)

            step_dx = 0
            step_dy = 0
            src_marks_shifted = src_marks.copy()  # default: no shift

            # offset src_img's detections so they can match with current camera img
            shift_x = 0
            shift_y = 0
            ...
            step_dx = shift_x
            step_dy = shift_y
            print(f"shift size (px): step_dx {step_dx}, step_dy={step_dy}")
            ...
            matched_dest, _, matched_src_shifted = match_alignment_markers_by_coordinates(
                dest_marks, src_marks, src_marks_shifted, img_h, img_w
            )
            print("\nmatched_dest_marks", matched_dest)
            print("matched_src_shifted_marks", matched_src_shifted)
            # check if we should proceed with error correction
            if len(matched_dest) < 1:
                print(
                    f"Warning: {len(matched_dest)} valid match(es) found, \
                    need at least 2 for rotation. Skipping correction."
                )
                return (0, 0, 0)
            ...
            if len(matched_dest) == 1:
                d0 = 0.0
            if abs(d0) >= 90.0:
                self.model.create_warning(
                    "Error: chip is rotated from previous layer. \
                    Please correct rotation and re-pattern. Skipping errors"
                )
                return (0, 0, 0)

            total_dx, total_dy, d0 = (dx, dy, d0)  # removed dx+step_x, removed dy+step_y
            print(f"error transform result (px): {round(total_dx)}, {round(total_dy)}, {d0}")

            return (round(total_dx), round(total_dy), d0)

        def slam_way_to_target(
            model,
            row: int,
            col: int,
            num_rows: int,
            ...
            - `num_cols`: total number of tile columns in pattern
            - `width, height`: steps to take in each direction
            - `partial_steps`: number of steps to take to get to next tiling location on stage
            - `layer`: layer 1 for first layer, 2, for next, etc.
            - `src_img, dest_img`: captures of destination (where you currently are),
                source (where you came from, or a pattern image for alignment feature)
            """
            error_y = 0  # steps
            error_x = 0  # steps
            h_direction = None
            ...
            for _ in range(num_iterations):
                ...
                h_direction, v_direction, step_x, step_y = get_next_tile_vector(
                    row, col, width, height, num_rows, num_cols, num_iterations, error_x, error_y
                )
                print(
                    f"\nnext tile vector returned: h_direction={h_direction}, \
                    v_direction={v_direction}, step_x={step_x}, step_y={step_y}"
                )

                # calculate relative position and move there
                dx_step = math.floor((step_x if h_direction == 'right' else -step_x) if h_direction != None else 0)
                dy_step = math.floor(
                    (step_y - 15 if v_direction == 'down' else -step_y + 15) if v_direction != None else 0
                )  # y-axis needs some leeway, it tends to overstep down

                # move the stage to next half-step + error corrected coordiantes
                print("moving a stride...")
                self.model.move_relative({'x': -dx_step, 'y': -dy_step})
                self.model.non_blocking_delay(1.0)

                # detect alignment errors from stage movement
                self.model.autofocus(blue_only=False, search=4, start=self.model.stage_setpoint[2])
                wait()

                dest_img = self.model.camera_image.copy()
                self.model.non_blocking_delay(0.5)

                # fetch alignment error
                dx_step = abs(dx_step)
                dy_step = abs(dy_step)
                print(
                    f"\nCalling alignment_errors with h={h_direction}, v={v_direction}, \
                                dx_step {dx_step} and dy_step {dy_step}, layer {layer}"
                )
                dx, dy, dr = snake_pattern_alignment_errors(
                    model,
                    dest_img,
                    src_img,
                    h_direction,
                ...
                print(f"alignment_errors returned with derivatives (pixels): {dx}, {dy}, {dr}")

                if dx == None and dy == None and dr == None:
                    dx, dy, dr = 0, 0, 0
                    self.model.create_warning("Couldn't find alignment markers. Warning: Tiling may fail.")

                # store errors so next movement accomodates for them
                error_x = round(dx * self.params.px_to_step_x)  # scale pixels to number of steps
                error_y = round(dy * self.params.px_to_step_y)  # scale pixels to number of steps
                print(f"transform is: {error_x}, {error_y}, with rotation {dr} (steps)")

                # set src to dest
                src_img = dest_img.copy()

            # fine-alignment
            print("\nFine alignment")
            src_img = self.model.camera_image.copy()
            dest_img = self.params.align_image.copy()
            print("Calling alignment_errors")
            dx, dy, dr = snake_pattern_alignment_errors(
                model,
                dest_img,
                src_img,
            ...
            print(f"alignment_errors returned with {dx}, {dy}, {dr}")
            ...
            error_x = round(dx * self.params.px_to_step_x)  # scale pixels to number of steps
            error_y = round(dy * self.params.px_to_step_y)  # scale pixels to number of steps
            error_x_img = (dx * self.params.px_to_step_x) * 1.0 / step_to_projection_pixels_x
            ...
            print(f"fine transform (projection): {error_x_img}, {error_y_img}, with rotation {dr} (steps)")
            ...
            while tries < 10:
                ...
                if (
                    can_exit
                    and abs(error_x) <= self.params.step_error_threshold_x
                    and abs(error_y) <= self.params.step_error_threshold_y
                ):
                    print("exiting slam loop and adjusting image position")

                    self.model.set_image_position(error_x_img, error_y_img, dr)
                    self.model.non_blocking_delay(0.5)
                    dest_img = self.params.align_image.copy()
                    return (self.model.camera_image.copy(), dest_img)

                # adjust
                print(f"\ntry {tries}: move_relative error_x{-error_x}, and error_y={error_y}")
                self.model.move_relative({'x': -error_x, 'y': error_y})
                self.model.non_blocking_delay(0.5)

                # capture and detect
                src_img = self.model.camera_image.copy()
                print("Calling snake_pattern_alignment_errors")
                dx, dy, dr = snake_pattern_alignment_errors(
                    model,
                    dest_img,
                    src_img,
                ...
                print(f"snake_pattern_alignment_errors reuturned iwth {dx}, {dy}, {dr}")

                if dx == None and dy == None and dr == None:
                    print("moving back a stride...")
                    # move back
                    self.model.move_relative({'x': error_x, 'y': -error_y})
                    self.model.non_blocking_delay(0.5)
                ...
                tries += 1
                if tries == 10:
                    print(f"\nfine alignment did not converge at tile ({row},{col})")
        ...
        def split_image_with_overlap(
            ...
            ####################### Rachel Insertion ############################################
            self.overlay_w_px = overlap_x  # pixels_digital
            self.overlay_h_px = overlap_y  # pixels_digital
            ...
            self.params.num_cols = len(x_positions)
            self.params.prefix_path = os.path.join(output_dir, "")
        ...
        def on_begin():
            ...
            print("doneset pattern")

            print("on_begin storing align_image")
            self.params.align_image = model.camera_image.copy()
            print("on_begin done align_image")

            src_img = model.camera_image.copy()  # (1200, 1840, 3)
            dest_img = None
            prev_row = 0
            prev_col = 0
            ...
            overlay_w_steps = self.overlay_w_px * digital_to_cam_view + self.leeway_w_steps
            overlay_h_steps = self.overlay_h_px * digital_to_cam_view + self.leeway_h_steps
            print(f"overlay_w_steps: {overlay_w_steps}, overlay_h_steps:{overlay_h_steps}")

            self.params.stride_x = int((self.projection_width_steps - overlay_w_steps) * self.params.ratio)
            self.params.stride_y = int((self.projection_height_steps - overlay_h_steps) * self.params.ratio)
            ...
            for row in range(self.params.num_rows):
                ...
                for col in range_order:
                    ...
                    if not (row == 0 and col == 0):
                        print(f"\nSlam_way_to_target: for {row}, {col} from {prev_row}, {prev_col}")
                        self.model.autofocus(blue_only=False, search=6, start=self.model.stage_setpoint[2])
                        wait()

                        src_img = self.model.camera_image.copy()
                        src_img, dest_img = slam_way_to_target(
                            model.model,
                            prev_row,
                            prev_col,
                    ...
                    prev_x, prev_y, _ = self.model.hardware.stage.get_position()
                    positions[f"{row}_{col}"] = (prev_x, prev_y)

                    pattern_path = f"{self.params.prefix_path}tile_{row},{col}.png"
                    print(f"\nUploading pattern: {self.params.prefix_path}tile_{row},{col}.png")
                    pattern_img = Image.open(f"{self.params.prefix_path}tile_{row},{col}.png")
                    ...
                    if row == 0 and col == 0:
                        model.move_relative({"z": -1 * self.red_to_uv_offset})  # first one starts from pattern
                    else:
                        model.move_relative({"z": -1 * (self.red_to_uv_offset - self.red_to_pattern_offset)})

                    model.non_blocking_delay(2.0)
                    ...
                    model.set_red_focus_source(RedFocusSource.SOLID)
                    self.model.set_image_position(0, 0, 0)
        ...
        self.segment_images_button = ttk.Button(
            self.frame,
            text="Segement Images",
            command=segment,
            state="disabled" if self.segmented else "enabled",
        )
...
class TilingCheckFrame:
    ...
    def capture_and_stitch(self):
        ...
        self.tile_width, self.tile_height = (
            3840,
            2160,
        )  # in pixels, defined in TilingFrame
    ...
    def takeAndStitchMapImages(self, stride_x, stride_y, crop_x, crop_y):
        ...
        for row in range(num_rows):
            ...
            self.event_dispatcher.move_absolute({"x": first_x, "y": current_y, "z": orig_z})
            ...
            for idx, col in enumerate(col_range):
                ...
                if idx > 0:  # idx = 0 first one don't need to move
                    self.event_dispatcher.move_absolute({"x": current_x, "y": current_y, "z": orig_z})
        ...
        self.event_dispatcher.move_absolute({"x": orig_x, "y": orig_y, "z": orig_z})
...
class MapFrame:
    def __init__(self, parent, event_dispatcher: EventDispatcher, canvas_size: int = 350):
        ...
        self.capture_button = ttk.Button(self.frame, text="Capture & Stitch", command=self.capture_and_stitch)
        self.capture_button.grid(row=0, column=0)
    ...
    def capture_and_stitch(self):
        ...
        preprocessed_imgs = self.preprocess_images(
            captures, settings=TilePreprocessSettings(gaussian_kernel_size=(7, 7))
        )
    ...
    def image_alignment(self, dst_img, src_img, display=False):
        ...
        if display:
            ...
            # draw matches
            draw_params = dict(
                matchColor=None,  # draw matches in green color
        ...
        for row in range(0, rows):
            ...
            for col in col_range:
                ...
                if M is None:
                    ...
                    print("alignment algorithm failed, using expected dx and dy")
...
class MultiLayerAlignFrame:
    def __init__(self, parent, event_dispatcher: EventDispatcher):
        ...
        self.capture_button = ttk.Button(
            self.frame, text="Detect Alignment Markers", command=self.detect_alignment_markers
        )
        self.capture_button.grid(row=0, column=0)
    ...
    def detect_alignment_markers(self):
        ...
        digital_marks = detect_marks_for_slam(
            processed_digital_pattern,
            self.event_dispatcher.model,
            orig_h,
            orig_w,
            threshold=STITCHED_CONFIDENCE_THRESHOLD,
        )

        processed_stitched_pattern, orig_h, orig_w = rf_detr_preprocess(stitched_image, layer=2)
        stitched_marks = detect_marks_for_slam(
            processed_stitched_pattern,
            self.event_dispatcher.model,
            orig_h,
            orig_w,
            threshold=STITCHED_CONFIDENCE_THRESHOLD,
        ...
        self.event_dispatcher.move_relative(
            {
                "x": x_scale * starting_mark["x"] + padding_x,
                "y": -y_scale * starting_mark["y"] + padding_y,
            }
        )
        self.event_dispatcher.non_blocking_delay(0.5)
...
class CanvasMapFrame:
    ...
    def _draw_pattern_marker(self, x_um, y_um):
        ...
        marker = self.canvas.create_rectangle(x1_px, y1_px, x2_px, y2_px, fill='#7BB7B7', width=0)
...
class LithographerGui:
    ...
    def __init__(self, config: LithographerConfig):
        ...
        # Map (top)
        self.map = CanvasMapFrame(self.top_panel, self.event_dispatcher, canvas_size=int(350 * ui_scale))
        ...
        self.global_settings_frame = GlobalSettingsFrame(
            self.bottom_panel, self.event_dispatcher, config.alignment.enabled
        )
        ...
        def on_start():
            self.camera.start()
            self.event_dispatcher.enter_red_mode(
                mode_switch_autofocus=False
            )  # ensure exposure settings are correctly set
    ...
    def _on_resize(self, event):
        ...
        self._pending_resize = self.root.after(150, lambda: self._apply_resize(event.width, event.height))
...
def main():
    ...
    config_path = filedialog.askopenfilename(title="Select a config file (default is default.toml)", filetypes=ftypes)
    ...
    if projector_config.get("dlpc_enabled", False):
        try:
            ...
            print(f"Set LED drive levels: red={SAFE_DEFAULT_LEVEL}, green={SAFE_DEFAULT_LEVEL}, blue(UV)={uv_level}")
        except Exception as e:
