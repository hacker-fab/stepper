...
class EventDispatcher(StepperEngine):
    """GUI adapter for StepperEngine, binding Tkinter UI events, dialogs, and mainloop pumps."""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            dlpc=dlpc,
            delay_func=self.non_blocking_delay,
...
class GlobalSettingsFrame:
    def __init__(self, parent, event_dispatcher: EventDispatcher, enable_detection: bool = False):
        ...
        def do_align():
            ...
            dx, dy = compute_standard_alignment_offset(
                markers, w, h, event_dispatcher.config.alignment
            )
...
class TilingFrame:
    def __init__(self, parent, model: EventDispatcher):
        ...
        def do_align_tiling(edge):
            ...
            markers, _ = detect_alignment_markers_tiling(model.model, model.camera_image, edge=edge)
            if len(markers) == 0:
                print("No markers detected.")
                return

            dx, dy = compute_tiling_alignment_offset(markers, w, h, model.config.alignment)
            if dx < 10 or dy < 10:
        ...
        def split_image_with_overlap(
            ...
            x_count, y_count, tile_count, (img_w, img_h) = split_image_with_overlap(
            ...
            self.x_settings.amount_var = x_count
            self.y_settings.amount_var = y_count
