...
def find_displacement(ref, input_frame, scale_factor=0.25, MIN_MATCH_COUNT=10):
    ...
    if len(good) > MIN_MATCH_COUNT:
        ...
        pts = np.float32([[0, 0], [0, h - 1], [w - 1, h - 1], [w - 1, 0]]).reshape(
            -1, 1, 2
        )  # top-left, bottom-left, bottom-right, top-right; 4 corner points at img1
        dst = cv2.perspectiveTransform(pts, M)  # Transform to img2 use M
        img2 = cv2.UMat.get(img2)
        imganno = cv2.polylines(
            np.copy(img2), [np.int32(dst)], True, 255, 3, cv2.LINE_AA
        )  # Draw White rectangle dst on img2
