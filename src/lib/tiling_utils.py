...
def estimate_transform(dest: np.ndarray, src: np.ndarray) -> tuple[float, float, float]:
    ...
    if abs(rotation_deg) >= 90.0:
        raise ValueError(
            f"Estimated rotation {rotation_deg:.1f}° exceeds 90° — chip is likely misloaded or detection failed."
        )
...
def get_next_tile_vector(
    ...
    if is_row_transition is True:  # moving to different row
        v_direction = 'down'
        h_direction = None
        ...
        v_direction = None
    print(
        f"width={width}, error_x={error_x}, num_steps={num_steps}, \
                                    height={height}, error_y={error_y}"
    )
...
def match_alignment_markers_by_coordinates(dest_marks, src_marks, src_marks_shifted, img_h, img_w):
    ...
    for r, c in zip(row_ind, col_ind):
        ...
        print(
            f"  cam[{r}]={dest_marks[r]} → pat[{c}]={src_marks_shifted[c]}  \
                                                dist={dist:.1f}px {status}"
        )
...
def extract_rectangle(img, display=False):
    ...
    valid = [c for c in contours if 0.02 * h * w < cv.contourArea(c) < 0.97 * h * w]
    if not valid:
