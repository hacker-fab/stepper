...
class Stage_Controller():
  ...
  __last_dir__: dirs_t = ('+','+')
  #endregion
  def calibrate(self, step_size: tuple[float|int, float|int]|float|int,
    ...
    def bad_location(loc: tuple[float,float]) -> bool:
      if(loc[0] < 0 or loc[0] > 1 or loc[1] < 0 or loc[1] > 1):
        if(self.debug != None):
          self.debug.error("Clicked out of bounds: "+str(loc)+"\n  Aborting calibration")
          return True
      return False
  ...
  def goto(self, location: tuple[float,float] | None = None):
    ...
    # get delta
    delta: list[float] = list(mult(self.__conversion_ratio__, sub(location, self.__calibrate_point__)))
...
class Multi_Stage():
  ...
  def __init__(self,
    ...
    for name in names:
      self.__controllers__[name] = Stage_Controller(debug = debug,
                                                    verbosity=verbosity,
  ...
  def toggle(self, names: None | str | list[str] = None,
    ...
    for name in name_list:
      ...
      if(self.debug != None and self.verbosity > 0):
        self.debug.info(("Enabled " if name in self.__selected__ else "Disabled ")+name+" stage")     
  ...
  def calibrate(self, name: str | list[str],
                step_size: tuple[float,float],
                calibrate_backlash: Literal['None','symmetric','bidirectional'] = 'None',
                return_to_start: bool = True) -> None:
    if(type(name) == str):
      if(name not in self.__controllers__ and self.debug != None):
        ...
        self.__controllers__[name].calibrate(step_size=step_size,
      ...
      for n in name:
        if(n not in self.__controllers__ and self.debug != None):
          ...
          self.__controllers__[n].calibrate(step_size=step_size,
                                            calibrate_backlash=calibrate_backlash,
                                            return_to_start=return_to_start)
      return
    else:
...
class Func_Manager():
  ...
  def __init__(self, name:str = "Unnamed Func Manager",
               debug: Debug | None = None,
               ):
    self.name = name
    self.__funcs__ = {}
